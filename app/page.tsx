...
export default function Home() {
  ...
  return <main>
    ...
    <section className="latest section-pad" id="latest">
      <div className="section-heading"><div><p className="kicker">NOW PLAYING / {featured.number}</p><h2>Vinyl<br/><i>Nights</i></h2></div><p className="section-note">A soundtrack for the moment<br/>the city turns velvet.</p></div>
    ...
    <section className="duo" id="story"><div className="duo-copy"><p className="kicker light">MEET THE FREQUENCY</p><h2>Two souls.<br/>One <i>sunset.</i></h2><p>Selectors, dreamers and late-night drivers. Solar Velvet lives between coastal elegance and dancefloor nostalgia. Every mix is a new scene in the same endless summer.</p><div className="duo-stats"><span><strong>01</strong> Season</span><span><strong>∞</strong> Summer</span><span><strong>24/7</strong> Mood</span></div></div><div className="duo-portrait"><img src={asset('/solar-velvet-duo-sunset.png')} alt="The Solar Velvet duo overlooking the sea at sunset"/><span className="orbit orbit-one">THE SELECTORS</span><span className="orbit orbit-two">RIVIERA / 43.7°N</span></div></section>

    <section className="albums section-pad" id="albums"><div className="section-heading albums-title"><div><p className="kicker">THE ARCHIVE</p><h2>Sunset<br/><i>sessions</i></h2></div><p className="section-note">A growing collection of<br/>chill disco journeys.</p></div><div className="album-grid">{releases.map((release)=><a className="album album-live" href={release.youtube} target="_blank" rel="noreferrer" key={release.number}><div className="album-art"><img src={release.albumImage} alt={`${release.title} CD artwork`} style={{transform:`scale(${release.discScale})`}}/><span><Play size={22} fill="currentColor"/></span></div><p><b>{release.title}</b><small>{release.year} · EPISODE {release.number}</small></p></a>)}{upcomingReleases.map((release)=><article className="album album-upcoming" key={release.number}><div className="album-art"><img src={release.albumImage} alt={`${release.title} CD artwork`} style={{transform:`scale(${release.discScale})`}}/><i>COMING SOON</i></div><p><b>{release.title}</b><small>{release.year} · EPISODE {release.number}</small></p></article>)}{futureSets.map((title,index)=><article className="album album-soon" key={title}><div className="album-art"><span className="soon-number">0{index+4}</span><i>COMING SOON</i></div><p><b>{title}</b><small>UPCOMING SESSION</small></p></article>)}</div></section>
