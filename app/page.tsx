...
const releases = [{ title: 'Riviera Nights', number: '001', year: '2026', mood: 'Golden hour · Coastal drive', image: '/riviera-nights.png', youtube: 'https://www.youtube.com/' }];
const futureSets = ['Velvet After Dark', 'Palm Radio', 'Midnight Coast'];
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="albums section-pad" id="albums"><div className="section-heading albums-title"><div><p className="kicker">THE ARCHIVE</p><h2>Sunset<br/><i>sessions</i></h2></div><p className="section-note">A growing collection of<br/>chill disco journeys.</p></div><div className="album-grid"><a className="album album-live" href={featured.youtube} target="_blank" rel="noreferrer"><div className="album-art"><img src={featured.image} alt="Riviera Nights cover"/><span><Play size={22} fill="currentColor"/></span></div><p><b>{featured.title}</b><small>{featured.year} · EPISODE {featured.number}</small></p></a>{futureSets.map((title,index)=><article className="album album-soon" key={title}><div className="album-art"><span className="soon-number">0{index+2}</span><i>COMING SOON</i></div><p><b>{title}</b><small>UPCOMING SESSION</small></p></article>)}</div></section>
